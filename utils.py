...
class lattice:
      ...
      for i, k in enumerate(self.kpath_ribbon):
        if method=='colpa':
          ...
          if i>=len(self.kpath_ribbon)/2-1:
                corrected = eigen[2*self.Ny-1].copy
                eigen[2*self.Ny-1] = eigen[2*self.Ny]
                eigen[2*self.Ny] = corrected 
    ...
    def plot_ribbon_dispersion(self, N_dest=None, yindex=0, ylim=[0,0], xlim=[0,0], dest_linewidth=2.0, norm_linewidth=1.0,label_size=2.0):
